...
- （原）未登录时，允许查看和下载简历
  - （现）未登录时，尝试通过标签页访问 / 通过网址直接访问，都会被重定向到 login 页面

- （原）页面预览与下载图片/PDF格式不一致，存在文字向下偏移问题
  - （现）更换了实现方式，预览与下载一致

### 0.2 ⚠️ 仍存在的 bug

- 登录页使用 **自动填充** 时，仍无法自动添加空格
- <span style="color:red;">注册信息丢失</span>：这次更新用本地数据覆盖了服务器数据，导致各位之前注册的账号 && 填写的表格数据都没有了😭
